...
sudo zypper in -y Mesa microsoft-edge-beta tmux ranger pinentry-gnome3 fontawesome-fonts font-manager fira-code-fonts font-manager brave-browser code-insiders vim python38 python38-devel python38-pip kitty alacritty fish zsh ripgrep opi ffmpeg mlocate sshpass cryfs wireguard-tools pwgen htop neovim command-not-found kubernetes1.24-client kubectx k9s go bat git openssh-common openssh-clients nodejs-common npm xdg-desktop-portal-gnome xdg-desktop-portal-wlr xdg-dbus-proxy xdg-desktop-portal-gtk xdg-desktop-portal-wlr xdg-user-dirs xdg-user-dirs-gtk 
